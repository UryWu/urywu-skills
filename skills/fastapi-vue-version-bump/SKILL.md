...
### v0.x → v1.0.0：内测结束后可以直接跳，不必凑"v0.99"

按 SemVer 规范，**v0.y.z 和 v1.0.0 之间有一条明确的红线**：

- **v0.x 阶段**：一切皆可随意更改，没有"向后兼容"承诺
- **v1.0.0 阶段**：必须提供向后兼容的稳定 API

内测结束、准备公测或正式上线时，**无论之前是 v0.2.3、v0.9.9 还是 v0.99.99**，都可以一步跨到 v1.0.0。v1.0.0 是个 **"稳定承诺"状态声明**，不是累加结果。

#### 行业实例

- **Vue.js**：从 v0.12.16 直接跳到 v1.0.0
- **Kubernetes**：从 v0.19.0 直接发布 v1.0.0
- **React**：在 v0.14.0 之后直接发布 v15.0.0（数字策略不同但同样跳过 1-14）

跳版本号在开源界是家常便饭。

#### 发布 v1.0.0 前的"三件套"红线检查

| 项 | 要做什么 |
|---|---|
| **API 稳定性锁定** | 对外公开的函数名 / 参数 / 返回值 / 配置格式，承诺 v1.x 内不做破坏性删改。如要改，必须等到 v2.0.0 |
| **完整 CHANGELOG** | 从 v0.2.3 跳过来可能累积了几十个 commit，Release Notes 里写明："从 v0.2.3 到 v1.0.0，新增 XXX、重构 YYY、修复 ZZZ" |
| **充分测试覆盖** | v1.0.0 意味着"生产可用"，核心功能必须有单元测试 / 集成测试通过 |

#### 包管理器的小红利

`npm install <pkg>` 默认装最新的 `1.x.x`，**不会**装 `0.x.x`（npm 把 `0.x` 视为不稳定）。所以从 v0.2.3 跳到 v1.0.0 后，用户通过常规命令就能直接拿到正式版——这是项目"进入稳定期"的强信号。`pip install`、`cargo install` 同样对 `0.x` 有保守默认。

#### 决策流程

```
内测反馈良好 + Bug 修复完毕 + API 不再变动
   ↓
直接打 v1.0.0 tag，发布
   ↓
若发现还需大改 API → 继续留在 v0.3.0、v0.4.0 迭代，直到"稳了"再一步跨到 v1.0.0
```

> 💡 **记住**：v1.0.0 是个 **"稳定承诺"**，不是 **"数字终点"**。只要承诺到位，随时可以跳。本 skill 的 `bump_version.{sh,ps1}` 处理 `major` 关键字时也是按 `MAJOR+1, MINOR/PATCH=0` 一步到位，不需要任何特殊标记。
